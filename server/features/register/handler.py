...
def create_agent_from_file_path(
    ...
    from server.core.llm_setup import create_litellm, setup_structured_llm

    # Create the dedicated structured LLM (STRUCTURED_LLM_*) once; it is reused
    # below for description auto-fill AND injected into the agent config for the
    # ColumnSelector.  Falls back to the main LLM when not configured.
    structured_llm = None
    try:
        # json_mode=True: description filling has no separate JSON-mode flag,
        # so force response_format at the instance level.
        structured_llm = setup_structured_llm(json_mode=True)
    except Exception:
        logger.warning("Failed to create structured LLM, falling back", exc_info=True)
        structured_llm = None
    ...
    if pandasai_config.auto_fill_descriptions and df.schema and df.schema.columns:
        ...
        _desc_llm = structured_llm
        # If no structured LLM is configured, build one from the agent LLM config.
        if _desc_llm is None and llm_config.api_key and llm_config.base_url:
            try:
    ...
    # Inject the dedicated structured LLM (created above in step 2c) into the
    # agent config for the ColumnSelector.  When None, the ColumnSelector falls
